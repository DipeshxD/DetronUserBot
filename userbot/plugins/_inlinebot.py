    ...
    buttons.append(
        [
            custom.Button.inline(
               f"Bᴀᴄᴋ {mafia_emoji}", data=f"page({(max_pages - 1) if page == 0 else (page - 1)})"
            ),
            custom.Button.inline(
               f"•{mafia_emoji} ❌ {mafia_emoji}•", data="close"
            ),
            custom.Button.inline(
               f"{mafia_emoji} Nᴇxᴛ", data=f"page({0 if page == (max_pages - 1) else page + 1})"
            ),
        ]
    )
...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "@DetronBot_Support":
            rev_text = query[::-1]
            veriler = button(0, sorted(CMD_HELP))
            result = await builder.article(
                f"Hey! Only use .help please",
                text=f"**Running DᴇᴛʀᴏɴBᴏᴛ**\n\n__Number of plugins installed__ :`{len(CMD_HELP)}`\n**page:** 1/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False,
            )
            ...
            result = builder.article(
                ...
                buttons=[
                    [
                        custom.Button.url("🔥 CHANNEL 🔥", "https://t.me/DetronBot_Support"),
                        custom.Button.url(
                            "⚡ GROUP ⚡", "https://t.me/DetronBot_Support"
                        ),
                    ],
                    [
                        custom.Button.url(
                            "✨ REPO ✨", "https://github.com/xdipesh/DetronBot"),
                        custom.Button.url
                    (
                            "🔰 OWNER 🔰", "https://t.me/DIPESH_XD" 
                    )
                    ],
                ],
                link_preview=False,
    ...
    async def page(event):
        if not event.query.user_id == bot.uid:
            return await event.answer(
                "HELLO THERE. PLEASE MAKE YOUR OWN DETRONBOT AND USE. © DetronBot_Support™",
                cache_time=0,
                alert=True,
            )
        ...
        await event.edit(
            f"**Legenday AF** [DᴇᴛʀᴏɴBᴏᴛ](https://t.me/DetronBot_Support) __Working...__\n\n**Number of modules installed :** `{len(CMD_HELP)}`\n**page:** {page + 1}/{veriler[0]}",
            buttons=veriler[1],
            link_preview=False,
        )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            await delete_mafia(event,
              "👑 DᴇᴛʀᴏɴBᴏᴛ Mᴇɴᴜ Pʀᴏᴠɪᴅᴇʀ ɪs ɴᴏᴡ Cʟᴏsᴇᴅ👑\n\n         **[© DᴇᴛʀᴏɴBᴏᴛ ™](t.me/DetronBot_Support)**", 5, link_preview=False
            )
        else:
            mafia_alert = "HELLO THERE. PLEASE MAKE YOUR OWN DETRONBOT AND USE. © DetronBot_Support™"
    ...
    async def Information(event):
        ...
        buttons.append([custom.Button.inline("Bᴀᴄᴋ", data=f"page({page})")])
        await event.edit(
    ...
    async def commands(event):
        ...
        await event.edit(
            result,
            buttons=[
                custom.Button.inline("Bᴀᴄᴋ", data=f"Information[{page}]({cmd})")
            ],
            link_preview=False,
        )
